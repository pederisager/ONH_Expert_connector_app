...
def test_chunker_splits_with_overlap() -> None:
    ...
    chunks = chunker.chunk_text(staff_slug="slug", text=words, source_url="https://example.com")
...
def test_chunker_respects_max_chunks() -> None:
    ...
    chunks = chunker.chunk_text(staff_slug="tester", text=words, source_url="https://example.com")
...
def test_chunker_handles_empty_text() -> None:
    chunker = Chunker()
    assert chunker.chunk_text(staff_slug="empty", text="   ", source_url="https://example.com") == []


@pytest.mark.parametrize("chunk_size, overlap", [(0, 0), (5, 5), (5, 6), (-1, 0), (5, -1)])
