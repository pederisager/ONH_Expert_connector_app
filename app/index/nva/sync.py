"""CLI for fetching NVA publications per staff member and materializing them for RAG ingestion."""
...
from .models import NvaPublicationRecord
from .normalizer import normalize_publication

LOGGER = logging.getLogger("nva.sync")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Fetch NVA publications for staff members and materialize them for RAG ingestion."
    )
    parser.add_argument(
        "--staff-csv",
        type=Path,
    ...
    parser.add_argument(
        "--raw-dir",
        type=Path,
        default=Path("data/nva/raw"),
        help="Directory where raw API responses are stored per employee.",
    )
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=Path("data/cache/nva"),
        help="Directory for HTTP cache. Disable with --force-refresh.",
    )
    parser.add_argument(
        "--key-file",
        type=Path,
        default=Path("nva_api_keys_test.json"),
        help="Path to JSON file containing clientId, clientSecret, and tokenUrl.",
    )
    parser.add_argument(
        "--base-url",
        default="https://api.test.nva.aws.unit.no",
        help="Base URL for the NVA API (default: test).",
    )
    parser.add_argument(
        "--per-page",
        type=int,
        default=100,
        help="Number of results fetched per page (1-1000).",
    )
    parser.add_argument(
        "--force-refresh",
        action="store_true",
...
def _information_score(record: NvaPublicationRecord) -> int:
    score = 0
    if record.abstract:
        score += 1000 + min(len(record.abstract), 2000)
    if record.venue:
        score += 200
...
def _dedup_key(record: NvaPublicationRecord) -> str:
    if record.title:
...
def _choose_richer(
    ...
    candidate_year = candidate.year or ""
    ...
    if candidate.abstract and not existing.abstract:
        return candidate
    if existing.abstract and not candidate.abstract:
        return existing

    return existing


def sync_nva_publications(
    *,
    ...
    with NvaClient(
        ...
        with output_path.open("w", encoding="utf-8") as sink:
            for member in staff_members:
                ...
                hits = client.get_publications_for_person(member.employee_id)
                stats.results_fetched += len(hits)
                ...
                dedup_map: dict[str, NvaPublicationRecord] = {}
                for hit in hits:
                    try:
                    ...
                    except Exception as exc:  # defensive guard for unexpected payloads
                        LOGGER.debug("Skipping hit for %s: %s", member.employee_id, exc)
                        continue

                    pub_id = record.publication_id
                    detail_path = person_raw_dir / f"{pub_id}.json"
...
def main(argv: list[str] | None = None) -> NvaSyncStats:
    parser = build_parser()
    ...
    if not staff_members:
        ...
        return NvaSyncStats(staff_skipped=skipped)

    cache = None
    if not args.force_refresh:
        cache = CacheManager(directory=args.cache_dir)

    api_key = NvaApiKey.from_file(args.key_file)

    config = NvaClientConfig(
