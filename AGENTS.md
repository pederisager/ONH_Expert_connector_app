...
- Railway deploys use `Dockerfile` + `railway.json` at repo root and install runtime dependencies from `requirements.deploy.txt` (CPU torch). Keep these files aligned with runtime/model changes, and keep the Docker build step that runs `python3 -m app.index.build --index-root data/index` so the container has a ready local vector index even though `data/index` is git-ignored.
